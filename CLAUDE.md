...
6. **Basmala (PRD §9)** : verset réel pour Al-Fatiha ; basmala virtuelle (`is_basmala_virtual: true`) ajoutée en calcul pour toute sourate importée sauf la 1 et la 9 (`needsVirtualBasmala`, logique par exclusion dans `lib/gematria/basmala.ts`). **Sourate 9 (At-Tawbah)** : seule sourate du Coran sans aucune basmala, ni réelle ni virtuelle (`hasNoBasmalaAtAll`, décision produit 2026-06-27) — son `total_value_with_basmala` est la somme des 129 versets réels uniquement (`calculateSurahTotal(..., false)`), et `total_value_without_basmala` reste `null` (distinction avec/sans basmala dénuée de sens, comme pour la sourate 1).
