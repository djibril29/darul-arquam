 ...
 * Hors périmètre — règle explicite à définir si elle est importée un jour
 * (PRD §9, CLAUDE.md règle d'or n°6). `needsVirtualBasmala` et
 * `hasRealBasmalaAsFirstVerse` renvoient alors `false` pour les deux,
 * ce qui fait échouer explicitement `updateSurahTotals()`.
 */
...
export function needsVirtualBasmala(surahNumber: number): boolean {
  return !hasRealBasmalaAsFirstVerse(surahNumber) && !SURAHS_WITHOUT_BASMALA.has(surahNumber);
}
